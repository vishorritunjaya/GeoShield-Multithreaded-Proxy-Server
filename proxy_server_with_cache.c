...
int handle_request(int clientSocket, ParsedRequest *request, char *tempReq)
{
	if (strcmp(request->method, "CONNECT") == 0) {
        // Handle CONNECT request (establish a tunnel between client and remote server)

        // Default remote server port for HTTPS is 443
        int server_port = 443;
        if (request->port != NULL) {
            server_port = atoi(request->port);  // Use the port specified in the request
        }

        // Step 1: Connect to the remote server (e.g., HTTPS server)
        int remoteSocketID = connectRemoteServer(request->host, server_port);
        if (remoteSocketID < 0) {
            return -1;  // Connection failed
        }

        // Step 2: Send the 200 Connection Established response to the client
        const char *response = "HTTP/1.1 200 Connection Established\r\n\r\n";
        send(clientSocket, response, strlen(response), 0);

        // Step 3: Forward data between client and remote server (this is typical for HTTPS)
        char buffer[MAX_BYTES];
        int bytes_recv;

        while ((bytes_recv = recv(clientSocket, buffer, MAX_BYTES, 0)) > 0) {
            // Send client data to the remote server
            send(remoteSocketID, buffer, bytes_recv, 0);

            // Receive data from remote server and send it back to the client
            bytes_recv = recv(remoteSocketID, buffer, MAX_BYTES, 0);
            send(clientSocket, buffer, bytes_recv, 0);
        }

        // Close the remote connection after data transfer
